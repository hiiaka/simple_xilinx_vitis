...
int main(int argc, char* argv[]) {
    ...
    size_t a_size_in_bytes = A_DATA_SIZE * sizeof(float);
    size_t b_size_in_bytes = B_DATA_SIZE * sizeof(float);
    size_t y_size_in_bytes = Y_DATA_SIZE * sizeof(float);
    ...
    for (int i = 0; i < A_DATA_SIZE; i++){ ptr_a[i] = (float)1.0; }
    for (int i = 0; i < B_DATA_SIZE; i++){ ptr_b[i] = (float)2.0; }
    ...
    cl::Kernel krnl(program, "krnl_multi_matrix");
    int narg = 0;
    ...
    for (int i = 0; i < A_DATA_SIZE; i++) { std::cout << "A[" << i << "] : " << ptr_a[i] << std::endl; }
    for (int i = 0; i < B_DATA_SIZE; i++) { std::cout << "B[" << i << "] : " << ptr_b[i] << std::endl; }
    for (int i = 0; i < Y_DATA_SIZE; i++) { std::cout << "Y[" << i << "] : " << ptr_y[i] << std::endl; }
