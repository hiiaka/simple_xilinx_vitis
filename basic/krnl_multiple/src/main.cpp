...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0 ; i < DATA_SIZE; i++) { ptr_in[i] = i; }
    ...
    cl::Kernel krnl_A(program, "krnl_add");
    int narg = 0;
    krnl_A.setArg(narg++, buffer_in);
    krnl_A.setArg(narg++, buffer_out);
    krnl_A.setArg(narg++, DATA_SIZE);

    q.enqueueMigrateMemObjects({buffer_in}, 0);
    q.enqueueTask(krnl_A);
    q.enqueueMigrateMemObjects({buffer_out}, CL_MIGRATE_MEM_OBJECT_HOST);
    q.finish();

    int tmp[DATA_SIZE];
    for (int i = 0; i < DATA_SIZE; i++) { tmp[i] = ptr_out[i]; }
    for (int i = 0; i < DATA_SIZE; i++) { ptr_in[i] = tmp[i]; }

    cl::Kernel krnl_B(program, "krnl_multi");
    narg = 0;
    krnl_B.setArg(narg++, buffer_in);
    krnl_B.setArg(narg++, buffer_out);
    krnl_B.setArg(narg++, DATA_SIZE);

    q.enqueueMigrateMemObjects({buffer_in}, 0);
    q.enqueueTask(krnl_B);
    q.enqueueMigrateMemObjects({buffer_out}, CL_MIGRATE_MEM_OBJECT_HOST);
    q.finish();
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        std::cout << "input[" << i << "] : " << i << ", result[" << i << "] :  " << ptr_out[i] << std::endl;
    }
